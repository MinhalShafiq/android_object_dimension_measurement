#include "network_server.h"
#include "point_cloud_processing.h"
#include "obb_calculator.h"

#include "DBSCAN_simple.h"
#include "DBSCAN_precomp.h"
#include "DBSCAN_kdtree.h"

#include <iostream>
#include <thread>
#include <sys/socket.h>
#include <netinet/in.h>
#include <unistd.h>
#include <arpa/inet.h>
#include <pcl/point_types.h>
#include <pcl/point_cloud.h>
#include <pcl/common/common.h>
#include <pcl/filters/voxel_grid.h>
#include <pcl/filters/statistical_outlier_removal.h>
#include <pcl/segmentation/sac_segmentation.h>
#include <pcl/filters/extract_indices.h>
#include <pcl/search/kdtree.h>

void processClient(int client_fd, SharedData& shared) {
    std::cout << "Client processing started" << std::endl;
    
    try {
        // Processing parameters.
        const float voxel_size        = 12.0f; 
        const int mean_k              = 20;
        const float stddev_thresh     = 1.0f;
        const float distance_thresh   = 25.0f;  
        const float cluster_tolerance = 20.0f;  
        const int min_cluster_size    = 500;
        const int max_cluster_size    = 307200;
        const float ground_margin     = -2.0f;   

        while (true) {
            // Variables for this frame - no shared access until the very end
            std::vector<CloudT::Ptr> frame_clusters;
            OBBData frame_obb;
            frame_obb.valid = false;
            bool frame_ready = false;
            
            try {
                // Network data reception
                uint32_t numFloats;
                ssize_t ret = recv(client_fd, &numFloats, sizeof(numFloats), MSG_WAITALL);
                if (ret != sizeof(numFloats)) {
                    std::cout << "Client disconnected (header)" << std::endl;
                    break;
                }
                numFloats = ntohl(numFloats);
                if (numFloats % 3 != 0 || numFloats == 0) {
                    std::cout << "Invalid data format" << std::endl;
                    break;
                }

                std::vector<float> buffer(numFloats);
                size_t bytesToRead = numFloats * sizeof(float);
                size_t totalRead = 0;
                while (totalRead < bytesToRead) {
                    ssize_t received = recv(client_fd,
                                            reinterpret_cast<char*>(buffer.data()) + totalRead,
                                            bytesToRead - totalRead,
                                            MSG_WAITALL);
                    if (received <= 0) {
                        std::cout << "Client disconnected (data)" << std::endl;
                        return; // Exit completely on socket error
                    }
                    totalRead += received;
                }
                if (totalRead != bytesToRead) {
                    std::cout << "Incomplete frame, skipping" << std::endl;
                    continue; 
                }

                // Build point cloud from received data
                pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
                cloud->width = numFloats / 3;
                cloud->height = 1;
                cloud->is_dense = false;
                cloud->points.resize(cloud->width);
                for (size_t i = 0; i < cloud->size(); ++i) {
                    cloud->points[i].x = buffer[i*3 + 0];
                    cloud->points[i].y = buffer[i*3 + 1];
                    cloud->points[i].z = buffer[i*3 + 2];
                }

                // Point cloud processing pipeline
                pcl::PointCloud<pcl::PointXYZ>::Ptr cleaned_cloud(new pcl::PointCloud<pcl::PointXYZ>);
                std::vector<int> indices;
                pcl::removeNaNFromPointCloud(*cloud, *cleaned_cloud, indices);
                if (cleaned_cloud->empty()) {
                    std::cout << "Empty cloud after NaN removal" << std::endl;
                    continue;
                }

                // Voxel grid downsampling
                pcl::PointCloud<pcl::PointXYZ>::Ptr downsampled(new pcl::PointCloud<pcl::PointXYZ>);
                pcl::VoxelGrid<pcl::PointXYZ> voxel;
                voxel.setInputCloud(cleaned_cloud);
                voxel.setLeafSize(voxel_size, voxel_size, voxel_size);
                voxel.filter(*downsampled);

                // Statistical outlier removal
                pcl::PointCloud<pcl::PointXYZ>::Ptr sor_cloud(new pcl::PointCloud<pcl::PointXYZ>);
                pcl::StatisticalOutlierRemoval<pcl::PointXYZ> sor;
                sor.setInputCloud(downsampled);
                sor.setMeanK(mean_k);
                sor.setStddevMulThresh(stddev_thresh);
                sor.filter(*sor_cloud);

                // Ground plane detection
                pcl::SACSegmentation<pcl::PointXYZ> seg;
                seg.setOptimizeCoefficients(true);
                seg.setModelType(pcl::SACMODEL_PLANE);
                seg.setMethodType(pcl::SAC_RANSAC);
                seg.setMaxIterations(1000);
                seg.setDistanceThreshold(distance_thresh);
                seg.setInputCloud(sor_cloud);

                pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
                pcl::ModelCoefficients coefficients;
                seg.segment(*inliers, coefficients);
                
                // Always create background visualization
                pcl::PointCloud<pcl::PointXYZRGB>::Ptr background_cloud(new pcl::PointCloud<pcl::PointXYZRGB>);
                pcl::copyPointCloud(*sor_cloud, *background_cloud);
                for (auto &p : *background_cloud) {
                    p.r = 255;
                    p.g = 255;
                    p.b = 255;
                }
                frame_clusters.push_back(background_cloud);
                
                if (inliers->indices.empty()) {
                    std::cout << "No ground plane detected" << std::endl;
                    frame_ready = true;
                    // No OBB will be computed, frame_obb.valid remains false
                } else {
                    // Ensure ground plane normal points upward
                    if (coefficients.values[2] > 0) {
                        for (auto &v : coefficients.values) v = -v;
                    }

                    // Extract objects above ground
                    pcl::PointCloud<pcl::PointXYZ>::Ptr objects_above(new pcl::PointCloud<pcl::PointXYZ>);
                    pcl::ExtractIndices<pcl::PointXYZ> extract;
                    extract.setInputCloud(sor_cloud);
                    extract.setIndices(inliers);
                    extract.setNegative(true);

                    pcl::PointCloud<pcl::PointXYZ> all_objs;
                    extract.filter(all_objs);
                    for (const auto& pt : all_objs) {
                        float dist_plane = coefficients.values[0] * pt.x +
                                           coefficients.values[1] * pt.y +
                                           coefficients.values[2] * pt.z +
                                           coefficients.values[3];
                        if (dist_plane > ground_margin) {
                            objects_above->push_back(pt);
                        }
                    }
                    objects_above->width = objects_above->size();
                    objects_above->height = 1;
                    objects_above->is_dense = true;
                    
                    if (objects_above->empty()) {
                        std::cout << "No objects above ground" << std::endl;
                        frame_ready = true;
                        // No OBB will be computed
                    } else {
                        // DBSCAN clustering
                        pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
                        tree->setInputCloud(objects_above);

                        DBSCANKdtreeCluster<pcl::PointXYZ> ec;
                        ec.setCorePointMinPts(15);
                        ec.setClusterTolerance(cluster_tolerance);
                        ec.setMinClusterSize(min_cluster_size);
                        ec.setMaxClusterSize(max_cluster_size);
                        ec.setSearchMethod(tree);
                        ec.setInputCloud(objects_above);

                        std::vector<pcl::PointIndices> cluster_indices;
                        ec.extract(cluster_indices);
                        
                        if (cluster_indices.empty()) {
                            std::cout << "No clusters found" << std::endl;
                            frame_ready = true;
                            // No OBB will be computed
                        } else {
                            // Get previous OBB for comparison (SINGLE ATOMIC READ)
                            OBBData previous_obb;
                            {
                                std::lock_guard<std::mutex> lock(shared.data_mutex);
                                previous_obb = shared.obb;
                            }
                            
                            // Build cluster clouds
                            std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr> cluster_clouds;
                            for (size_t i = 0; i < cluster_indices.size(); ++i) {
                                pcl::PointCloud<pcl::PointXYZRGB>::Ptr cluster_rgb(new pcl::PointCloud<pcl::PointXYZRGB>);
                                pcl::copyPointCloud(*objects_above, cluster_indices[i].indices, *cluster_rgb);
                                cluster_clouds.push_back(cluster_rgb);
                            }
                            
                            std::cout << "Processing " << cluster_clouds.size() << " clusters" << std::endl;

                            // Cluster validation and selection (NO SHARED ACCESS)
                            size_t bestLabel = selectBestObjectCluster(cluster_clouds, previous_obb);
                            
                            if (bestLabel != SIZE_MAX) {
                                std::cout << "Selected cluster " << bestLabel << std::endl;
                                pcl::PointCloud<pcl::PointXYZRGB>::Ptr best_cluster = cluster_clouds[bestLabel];

                                // Color the cluster purple
                                for (auto &pt : *best_cluster) {
                                    pt.r = 128;
                                    pt.g = 0;
                                    pt.b = 128;
                                }
                                
                                // Shadow removal and validation
                                pcl::PointCloud<pcl::PointXYZ>::Ptr best_xyz(new pcl::PointCloud<pcl::PointXYZ>);
                                pcl::copyPointCloud(*best_cluster, *best_xyz);
                                
                                std::cout << "Applying shadow removal..." << std::endl;
                                pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_xyz = removeShadowTail(best_xyz);
                                filtered_xyz = removeRemainingOutliers(filtered_xyz);
                                
                                // Final validation
                                ObjectValidationResult post_validation = validateObjectCluster(filtered_xyz);
                                if (post_validation.is_valid_object) {
                                    // Create final colored cluster
                                    pcl::PointCloud<pcl::PointXYZRGB>::Ptr final_cluster(new pcl::PointCloud<pcl::PointXYZRGB>);
                                    pcl::copyPointCloud(*filtered_xyz, *final_cluster);
                                    for (auto &pt : *final_cluster) {
                                        pt.r = 128;
                                        pt.g = 0;
                                        pt.b = 128;
                                    }

                                    // Compute OBB (NO SHARED ACCESS IN THESE FUNCTIONS)
                                    OBBData computed_obb = findMinimumOBB(filtered_xyz);
                                    if (computed_obb.valid) {
                                        computed_obb = extendOBBToGround(computed_obb, coefficients);
                                        
                                        // Apply temporal smoothing if we have previous data
                                        if (previous_obb.valid) {
                                            frame_obb = temporallyStableOBB(previous_obb, computed_obb);
                                        } else {
                                            frame_obb = computed_obb;
                                        }
                                        
                                        // Set timing information
                                        frame_obb.last_update_time = std::chrono::steady_clock::now();
                                        frame_obb.frames_since_valid_detection = 0;
                                        frame_obb.confidence_at_creation = post_validation.confidence_score;
                                        frame_obb.valid = true;
                                        
                                        // Add object to visualization
                                        frame_clusters.push_back(final_cluster);
                                        
                                        std::cout << "Valid OBB computed successfully!" << std::endl;
                                    } else {
                                        std::cout << "OBB computation failed" << std::endl;
                                    }
                                } else {
                                    std::cout << "Object invalid after filtering: " << post_validation.rejection_reason << std::endl;
                                }
                            } else {
                                std::cout << "No valid clusters found" << std::endl;
                            }
                            
                            frame_ready = true;
                        }
                    }
                }
                
            } catch (const std::exception& e) {
                std::cerr << "Frame processing error: " << e.what() << std::endl;
                frame_ready = true; // Mark frame as ready even with errors
                // frame_obb.valid remains false
            } catch (...) {
                std::cerr << "Unknown frame processing error" << std::endl;
                frame_ready = true; // Mark frame as ready even with errors
                // frame_obb.valid remains false
            }
            
            // IMMEDIATE BB UPDATE - NO TIMEOUT SYSTEM
            if (frame_ready) {
                // Simple logic: if we have a valid detection, use it; otherwise BB disappears immediately
                OBBData final_obb;
                
                if (frame_obb.valid) {
                    // We have a new valid detection - use it
                    final_obb = frame_obb;
                    std::cout << "OBB updated with new detection" << std::endl;
                } else {
                    // No valid detection - BB disappears immediately
                    final_obb.valid = false;
                    final_obb.frames_since_valid_detection = 0;
                    final_obb.confidence_at_creation = 0.0f;
                    std::cout << "No valid detection - BB hidden immediately" << std::endl;
                }
                
                // SINGLE ATOMIC WRITE TO SHARED DATA
                {
                    std::lock_guard<std::mutex> lock(shared.data_mutex);
                    shared.clusters = frame_clusters;
                    shared.obb = final_obb;
                    shared.new_data_available = true;
                }
                
                std::cout << "Frame complete - OBB: " << (final_obb.valid ? "VALID" : "INVALID") << std::endl;
            }
        }
        
    } catch (const std::exception& e) {
        std::cerr << "Fatal client error: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "Fatal unknown client error" << std::endl;
    }
    
    std::cout << "Client processing ended" << std::endl;
    close(client_fd);
}

void videoServer(SharedData& shared) {
    try {
        const int PORT = 9081;
        int server_fd = socket(AF_INET, SOCK_STREAM, 0);
        if (server_fd < 0) {
            std::cerr << "Failed to create video server socket" << std::endl;
            return;
        }
        int opt = 1;
        if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
            std::cerr << "setsockopt(SO_REUSEADDR) failed for video server" << std::endl;
        }
#ifdef SO_REUSEPORT
        if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)) < 0) {
            std::cerr << "setsockopt(SO_REUSEPORT) failed for video server" << std::endl;
        }
#endif

        sockaddr_in addr{};
        addr.sin_family = AF_INET;
        addr.sin_addr.s_addr = INADDR_ANY;
        addr.sin_port = htons(PORT);

        if (bind(server_fd, (sockaddr*)&addr, sizeof(addr)) < 0) {
            std::cerr << "Video server bind failed" << std::endl;
            close(server_fd);
            return;
        }

        if (listen(server_fd, 5) < 0) {
            std::cerr << "Video server listen failed" << std::endl;
            close(server_fd);
            return;
        }

        std::cout << "Video server listening on port " << PORT << std::endl;

        while (true) {
            try {
                sockaddr_in client_addr{};
                socklen_t len = sizeof(client_addr);
                int client_fd = accept(server_fd, (sockaddr*)&client_addr, &len);
                if (client_fd < 0) {
                    std::cerr << "Video server accept failed" << std::endl;
                    continue;
                }
                
                // ATOMIC video client management
                {
                    std::lock_guard<std::mutex> lock(shared.video_mutex);
                    if (shared.video_client_fd >= 0) {
                        std::cout << "Closing existing video client connection." << std::endl;
                        close(shared.video_client_fd);
                    }
                    shared.video_client_fd = client_fd;
                }
                
                std::cout << "New video client connected" << std::endl;
            }
            catch (const std::exception& e) {
                std::cerr << "Video server loop error: " << e.what() << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
            catch (...) {
                std::cerr << "Unknown video server loop error" << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
        }
    }
    catch (const std::exception& e) {
        std::cerr << "Video server thread error: " << e.what() << std::endl;
    }
    catch (...) {
        std::cerr << "Unknown video server thread error" << std::endl;
    }
}

void pointCloudServer(SharedData& shared) {
    try {
        const int PORT = 9080;
        int server_fd = socket(AF_INET, SOCK_STREAM, 0);
        if (server_fd < 0) {
            std::cerr << "Failed to create point cloud server socket" << std::endl;
            return;
        }
        int opt = 1;
        if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
            std::cerr << "setsockopt(SO_REUSEADDR) failed for point cloud server" << std::endl;
        }
#ifdef SO_REUSEPORT
        if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)) < 0) {
            std::cerr << "setsockopt(SO_REUSEPORT) failed for point cloud server" << std::endl;
        }
#endif

        sockaddr_in addr{};
        addr.sin_family = AF_INET;
        addr.sin_addr.s_addr = INADDR_ANY;
        addr.sin_port = htons(PORT);

        if (bind(server_fd, (sockaddr*)&addr, sizeof(addr)) < 0) {
            std::cerr << "Point cloud server bind failed" << std::endl;
            close(server_fd);
            return;
        }

        if (listen(server_fd, 5) < 0) {
            std::cerr << "Point cloud server listen failed" << std::endl;
            close(server_fd);
            return;
        }

        std::cout << "Point cloud server listening on port " << PORT << std::endl;

        while (true) {
            try {
                sockaddr_in client_addr{};
                socklen_t len = sizeof(client_addr);
                int client_fd = accept(server_fd, (sockaddr*)&client_addr, &len);
                if (client_fd < 0) {
                    std::cerr << "Failed to accept point cloud client connection" << std::endl;
                    continue;
                }
                
                // ATOMIC point cloud client management
                bool accept_connection = false;
                {
                    std::lock_guard<std::mutex> lock(shared.pointCloud_mutex);
                    if (shared.pointCloud_client_fd >= 0) {
                        std::cerr << "Point cloud client already connected. Rejecting new connection." << std::endl;
                        close(client_fd);
                        accept_connection = false;
                    } else {
                        shared.pointCloud_client_fd = client_fd;
                        accept_connection = true;
                    }
                }
                
                if (accept_connection) {
                    std::cout << "New point cloud client connected" << std::endl;
                    
                    // Launch client processing thread
                    std::thread([client_fd, &shared]() {
                        try {
                            processClient(client_fd, shared);
                        }
                        catch (const std::exception& e) {
                            std::cerr << "processClient thread error: " << e.what() << std::endl;
                        }
                        catch (...) {
                            std::cerr << "Unknown processClient thread error" << std::endl;
                        }
                        
                        // Reset the point cloud connection flag after the client disconnects.
                        {
                            std::lock_guard<std::mutex> lock(shared.pointCloud_mutex);
                            shared.pointCloud_client_fd = -1;
                        }
                        std::cout << "Point cloud client disconnected" << std::endl;
                    }).detach();
                }
            }
            catch (const std::exception& e) {
                std::cerr << "Point cloud server loop error: " << e.what() << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
            catch (...) {
                std::cerr << "Unknown point cloud server loop error" << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
        }
    }
    catch (const std::exception& e) {
        std::cerr << "Point Cloud server thread error: " << e.what() << std::endl;
    }
    catch (...) {
        std::cerr << "Unknown Point Cloud server thread error" << std::endl;
    }
}